			...
			<el-col :xs={span:24} :sm={span:11} :md="10" :lg="9" :xl="8">
				<div class="show_json_outer">
					...
					<div id="show_json" v-show="showResult">
						...
						<p>证号：{{showJson.card_id}}</p>
						<p>姓名：{{showJson.driver}}</p>
						<p>性别：{{showJson.sex}}</p>
						<p>国籍：{{showJson.nationality}}</p>
						<p>住址：{{showJson.address}}</p>
						<p>出生日期：{{showJson.birthday}}</p>
						<p>初次领证日期：{{showJson.issue_date}}</p>
						<p>准驾车型：{{showJson.be_class}}</p>
						<p>有效期限：{{showJson.valid_start}}至{{showJson.valid_end}}</p>
					</div>
					<div class="drive_wrong"  v-show="isNotDrive">
						<p>请上传驾驶证照片！</p>
					</div>
				</div>
			</el-col>
    ...
    export default {
        ...
        methods: {
            uploadImage(e){
                ...
                formData.append('channel_id', 7);
                $.ajax({
                    url: this.api+"/api/v1/ocr/get_driving_licence_ocr/",
                    type: "post",
                    data: formData,
...
<style scoped>
	...
	.drive_wrong{text-align: center;height: 350px;line-height:350px;color: #ff4949;}
